...
def validate_user(user):
    ...
    resul = ' '.join(resul)
    print("Employer: validate_user() Cuenta iniciada...")
    start_time = time.time()
    socket.send(str(resul).encode())
    #socket.send(str(123).encode())
    ret = socket.recv().decode()
    elapsed_time = time.time() - start_time
    print("Employer: validate_user() Tiempo transcurrido: ", str(elapsed_time))
    return ret
...
def createUser(emplo):
    ...
    socket = context.socket(zmq.REQ)
    socket.connect("tcp://25.12.72.51:5555")
    print("Employer: createUser() Cuenta iniciada...")
    start_time = time.time()
    socket.send(str(emplo).encode())
    ret = socket.recv().decode()
    elapsed_time = time.time() - start_time
    print("Employer: createUser() Tiempo transcurrido: ", str(elapsed_time))
    return ret
